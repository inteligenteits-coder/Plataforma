...
async function verifyCode(nivel) {
...
function populateWeekSelector() {
  ...
  weekSelect.innerHTML = Object.keys(weeks).map(key => {
    const w = weeks[key];
    const num = w.week || key;
    return `<option value="${key}">Semana ${num}: ${w.title}</option>`;
  }).join('');
}

function renderTeoriaEstilizada(data, semanaId, userData) {
  const container = document.getElementById('teoria-container');
  if (!container) return;
  container.innerHTML = '';
  if (!data.teoria) {
    container.innerHTML = '<p style="color: var(--text-muted);">No hay teoría disponible para esta semana.</p>';
    return;
  }
  const niveles = ['basico', 'alto', 'superior'];
  let html = '<div class="teoria-cards-container" style="display: flex; flex-direction: column; gap: 20px;">';
  niveles.forEach(nivel => {
    const tData = data.teoria[nivel];
    if (!tData) return;
    const isClaimed = userData.teoria?.[`teoria_leida_${semanaId}_${nivel}`] === true;
    const colorNivel = nivel === 'basico' ? 'var(--nivel-basico)' : (nivel === 'alto' ? 'var(--nivel-alto)' : 'var(--nivel-superior)');
    let quizHtml = '';
    if (tData.quiz) {
      const quizId = `quiz-${semanaId}-${nivel}`;
      const optionsHtml = tData.quiz.opciones.map((opt, idx) => {
        return `<label style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; cursor: pointer; color: var(--text-light); font-size: 0.9rem;"><input type="radio" name="${quizId}" value="${idx}" data-correct="${tData.quiz.correcta}" style="cursor: pointer;"><span>${opt}</span></label>`;
      }).join('');
      quizHtml = `<div class="quiz-box" style="margin-top: 15px; padding: 12px; background: rgba(128,128,128,0.05); border-radius: 8px; border: 1px solid var(--border-color);"><div style="font-weight: 600; font-size: 0.9rem; margin-bottom: 10px; color: var(--wokwi-blue);"><i data-lucide="help-circle" style="display:inline-block; width:16px; height:16px; vertical-align:middle; margin-right:5px;"></i> Pregunta de Validación:</div><div style="font-size: 0.9rem; margin-bottom: 10px; font-weight: 500;">${tData.quiz.pregunta}</div><div style="display: flex; flex-direction: column;">${optionsHtml}</div><div class="quiz-feedback" style="margin-top: 8px; font-size: 0.85rem; font-weight: 600; display: none;"></div></div>`;
    }
    let btnHtml = '';
    if (isClaimed) {
      btnHtml = `<button id="btn-teoria-${nivel}" class="btn-verify reclamado" style="margin-top: 12px; width: 100%;" disabled>✔️ Recompensa Reclamada</button>`;
    } else {
      const btnStyle = tData.quiz ? 'display: none; margin-top: 12px; width: 100%;' : 'margin-top: 12px; width: 100%;';
      btnHtml = `<button id="btn-teoria-${nivel}" class="btn-verify" style="${btnStyle}" data-action="reclamarMonedas" data-semana="${semanaId}" data-nivel="${nivel}" data-cantidad="${tData.monedas}">${tData.quiz ? 'Reclamar Recompensa' : 'Marcar como leído'} (🪙 ${tData.monedas})</button>`;
    }
    html += `<div class="teoria-card" style="border-left: 4px solid ${colorNivel}; padding: 15px; background: rgba(128, 128, 128, 0.03); border-radius: 8px; border-top: 1px solid var(--border-color); border-right: 1px solid var(--border-color); border-bottom: 1px solid var(--border-color);"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;"><h4 style="margin: 0; color: ${colorNivel}; font-size: 1.1rem; font-weight: 600;">${tData.titulo}</h4><span style="font-size: 0.85rem; background: rgba(227, 179, 65, 0.1); color: #e3b341; padding: 3px 8px; border-radius: 12px; font-weight: 600; border: 1px solid rgba(227, 179, 65, 0.2);">🪙 +${tData.monedas}</span></div><p style="color: var(--text-light); font-size: 0.95rem; line-height: 1.5; margin: 0 0 10px 0;">${tData.contenido}</p>${tData.ejemplo ? `<div style="background: var(--code-bg); padding: 10px; border-radius: 6px; border: 1px solid var(--border-color); font-family: 'Fira Code', monospace; font-size: 0.85rem; color: #e1e4e8; white-space: pre-wrap; margin-bottom: 10px;"><code>${tData.ejemplo}</code></div>` : ''}${quizHtml}${btnHtml}</div>`;
  });
  html += '</div>';
  ...
  container.querySelectorAll('input[type="radio"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
      ...
      const correctIdx = parseInt(e.target.dataset.correct);
      const card = e.target.closest('.teoria-card');
      const feedback = card.querySelector('.quiz-feedback');
      const nivel = card.querySelector('[id^="btn-teoria-"]').id.split('-').pop();
      const btn = card.querySelector(`#btn-teoria-${nivel}`);
      if (selectedIdx === correctIdx) {
        feedback.style.display = 'block';
        feedback.style.color = 'var(--accent)';
        feedback.innerHTML = '¡Correcto! Excelente deducción.';
        if (btn) btn.style.display = 'block';
      } else {
        feedback.style.display = 'block';
        feedback.style.color = 'var(--error-color)';
        feedback.innerHTML = 'Incorrecto. Revisa la teoría e intenta de nuevo.';
...
function setupEventListeners() {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        switch (action) {
            ...
            case 'verCodigo':
                ...
                break;
            case 'enviarFeedback':
                import('./modules/teacher.js').then(m => m.enviarFeedback(target.dataset.uid, target.dataset.nombre));
                break;
            case 'exportPiarPDF':
                import('./modules/teacher.js').then(m => m.exportPiarPDF());
                break;
        }
    });
    ...
    // Event listeners for PIAR profile change
    document.body.addEventListener('change', (e) => {
        const target = e.target.closest('[data-action="setPerfilPiar"]');
        if (target) {
            const uid = target.dataset.uid;
            const perfil = target.value;
            import('./modules/teacher.js').then(m => m.setPerfilPiar(uid, perfil));
        }
    });
}


// Exposición de funciones al ámbito global para controladores inline onclick
...
window.comprarPowerup = comprarPowerup;
